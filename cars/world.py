...
from .base.track import plot_map
from .base.utils import CarState, to_px, rotate, intersect_ray_with_segment, draw_text, angle
...
class SimpleCarWorld(World):
    ...
    def set_agents(self, agents=1, agent_class=None):
        ...
        self.agent_states = {a: CarState(pos, vel, heading)
    ...
    def reward(self, state, collision):
        ...
        idle_penalty = 0 if abs(
            state.velocity) > MIN_SPEED else - IDLENESS_PENALTY
        speeding_penalty = 0 if abs(
            state.velocity) < MAX_SPEED else - SPEEDING_PENALTY * abs(state.velocity)
        collision_penalty = - max(abs(state.velocity),
                                  0.1) * int(collision) * COLLISION_PENALTY

        return heading_reward * HEADING_REWARD + heading_penalty * WRONG_HEADING_PENALTY + collision_penalty \
    ...
    def run(self, steps=None):
        ...
        for i, agent in enumerate(self.agents):
            try:
                # TODO add date/time/ -> folder data/
    ...
    def vision_for(self, agent: SimpleCarAgent) -> List[float]:
        """
        Строит видение мира для каждого агента
        :param agent: car
        :return: list[float]
            car_speed
            car_angle (car_direction vs car_to_center)
            rays values
        """
        state = self.agent_states[agent]
        vision = [abs(state.velocity), np.sin(
            angle(-state.position, state.heading))]
        extras = len(vision)
        ...
        for i in range(agent.rays):
            ...
            for j in range(sectors):
                ...
                intersect = intersect_ray_with_segment(
                    (state.position, ray), inner_wall)
                intersect = abs(
                    intersect - state.position) if intersect is not None else np.infty
                if intersect < vision[-1]:
                    vision[-1] = intersect

                intersect = intersect_ray_with_segment(
                    (state.position, ray), outer_wall)
                intersect = abs(
                    intersect - state.position) if intersect is not None else np.infty
                if intersect < vision[-1]:
                    vision[-1] = intersect
            ...
            assert vision[-1] < np.infty, \
                "Something went wrong: {}, {}".format(
                    str(state), str(agent.chosen_actions_history[-1]))
    ...
    def _draw_ladar(self, sensors, state, scale):
        ...
        for s in sensors:
            end_pos = to_px(rect(s, ray) + state.position,
    ...
    def _update_display(self):
        ...
        # clear notifications from previous round
